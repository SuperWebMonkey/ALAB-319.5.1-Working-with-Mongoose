...
const gradeSchema = new mongoose.Schema({
  ...
  scores: [{ type: Number, requied: true }],
  avg: { type: Number, default: 0, required: false },
});

gradeSchema.index({ learner_id: 1 });
gradeSchema.index({ class_id: 1 });
gradeSchema.index({ scores: 1 });

gradeSchema.methods.getScores = function (cb) {
  return mongoose
    .model("grade")
    .find({ learner_id: this.learner_id, class_id: this.class_id }, cb);
};

gradeSchema.statics.findByLearnerId = function (learner_id) {
  return this.find({ learner_id });
};
